...
2. `supabase-migration-4-ordem-produtos.sql` — cria a coluna `ordem`, usada para os carrosséis mostrarem sempre o cardápio na sequência oficial (e não por data ou ordem alfabética).
3. `supabase-menu-update.sql` — substitui os produtos de exemplo pelo cardápio oficial completo (comidas e bebidas), já com a `ordem` de cada item preenchida.
